...
from .models import User, Organization, GeneralModel, Company
...
class BaseModelAdmin(admin.ModelAdmin):
    ...
    def has_view_permission(self, request, obj=None):
        ...
        base_perm = request.user.has_perm('%s.%s' % (opts.app_label, codename_view)) or request.user.has_perm('%s.%s' % (opts.app_label, codename_change))
        
        try:
            company_permission = obj.company==request.user.company
            if base_perm and company_permission:
                    True
...
class CustomUserAdmin(UserAdmin, BaseModelAdmin):
    ...
    fieldsets = (
        ...
        ('Organizations',{'fields':('organizations', 'guest_organizations', 'company')}),
    )
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('name', 'email', 'password1', 'password2', 'is_staff', 'is_admin', 'is_superuser', 'is_active', 'groups', 'organizations', 'company', 'guest_organizations')}
        ),
    )
